...
if(login_check() == true) {
   global $n;
        ...
        $stmt = new DB_Sql;
        $stmt->query("SELECT id, type, username, agentID, billable,company,shipable from agents where id = '$user_id'");
        $stmt->next_record();
		$agent_id = $stmt->f("agentID");
                $uname = $stmt->f("username");
                $utype = $stmt->f("type");
                $ubillable = $stmt->f("billable");
                $shipable = $stmt->f("shipable");
                $company = $stmt->f("company");
   ...
   if(!($utype == "main" && $ubillable=="Y") ){
	$ip=getenv ("REMOTE_ADDR");
        $tlog = new DB_Sql;
        $tlog->query("INSERT INTO bad_attempts (ip, msg, time) VALUES ('$ip', 'Try to directly access cc-less order.php agent:".$agent_id."', UTC_TIMESTAMP())");
...
<div align="center">
          ...
          <form method="post" id="oform" action="post.php">
	    ...
	    <input type="hidden" value="Agent" name="SUBMITTYPE">	
            <input type="hidden" value="Y" name="BILLTOAGENT">
            <input type="hidden" value="'.$shipable.'" name="SHIPFROMAGENT">
...
-->
              ...
              <tr>
                ...
                <td bgcolor="#ffffff" align="left"><select onchange="CalculateTotal();" onfocusout="CalculateTotal();" size="1" id="PACKAGE_TYPE" name="PACKAGE_TYPE" style="background-color:silver">
                  ...
                  <option value="Basic">Basic $(39.95)</option>
...
<br>
              ...
              <tr>
                ...
                <td bgcolor="#ffffff" align="left">';
                $companych = array();
                $checked_html = 'checked="on"';
                $companyarr = explode(",",$company);
                if(count($companyarr)==0 || $company==""){
                        print '<select onchange="sendwarning();" name="USERNAME">
                                    <option value="--select one--">--select one--</option>
                                <option value="Acanac">Acanac</option>
                                <option value="Distibutel">Distributel</option>
                                <option value="Xinflix">Xinflix</option>
                                <option value="Mustang Technologies">Mustang Technologies</option>
                                <option value="Others">Other</option>';
                }else if(count($companyarr)==1){
                        print '<select onchange="sendwarning();" name="USERNAME">
                                    <option value="'.$companyarr[0].'">'.$companyarr[0].'</option>';
                }else{
                        print '<select onchange="sendwarning();" name="USERNAME">
                                <option value="--select one--">--select one--</option>';
                        foreach($companyarr as $compen){
                                print '<option value="'.$compen.'">'.$compen.'</option>';
                        }
                        print '<option value="Others">Other</option>';
                }

print '	</select>
	<!--<font size="-2" color="grey"><span id="isp" name="isp"><br>You can order required Internet Service through:<br><a target="_blank" href="http://www.distributel.ca">Distributel</a><br><a target="_blank" href="http://www.acanac.com">Acanac Inc.</a><br><a target="_blank" href="http://www.xinflix.com">Xinflix Media Inc.</a><br></span><br>
</font>--></td>
...
echo           '   <tr>
                <td valign="top" bgcolor="#ffffff" align="left"> &nbsp;Number of required Set Top Box units:<font color="red">*</font><br>
                   &nbsp;
                  <br>                    <br></td>
                <td bgcolor="#ffffff" align="left"><select onchange="" onfocusout="" size="1" id="STB_COUNT" name="STB_COUNT">
                  <option value="--select one--">--select one--</option>
...
<script type="text/javascript">
	...
	<tr>
              ...
              <tr>
                ...
                <td bgcolor="#ffffff" align="left"><input id="AGENTID" name="AGENTID" value='.$agent_id.' readonly></td>
                </tr>
...
<a onclick="document.getElementById(\'informationcontent2\').style.display=\'none\';document.getElementById(\'informationcontent1\').style.display=\'block\';document.getElementById(\'information\').style.display=\'block\'"><img src=/images/Info_Icon.gif></a>
</span><br><br>
...
Month: <select name=birthdaym id=birthdaym style="width:75px;">
<option>
<option>Jan
<option>Feb
<option>Mar
...
&nbsp;Day: <select name=birthdayd id=birthdayd class="span1">
<option>';
for ($i=1;$i<=31;$i++){
        print "<option>".$i."\n";
}
print '</select>
&nbsp;Year:
<select name=birthdayy id=birthdayy class="span2">
<option>';
for ($i=2005;$i>=1910;$i--){
        print "<option>".$i."\n";
}
...
your birth date you automatically quality for service discounts or gifts on your anniversary.<br><br></td></tr></table></td>
                </tr>-->
              <tr>
...
<br></td>
		...
		<tr>
<!--		<tr>
                <td align="left" bgcolor="#ffffff" valign="top"><span style=""> &nbsp;DSL order ID (<font size=-1>if applicable:</font>)<br>
                  <br>
                </span></td>
                <td align="left" bgcolor="#ffffff"><input type = "text" name = "PROMO" id=PROMO size = "44" maxlength = "55" value = "" /></td>
                </tr>-->
			...
			<td valign="top" bgcolor="#ffffff" align="left">
				<div id="subtotal" name="subtotal"><script>//CalculateTotal()</script><table width="350px" cellspacing="0" cellpadding="0" bordercolor="#000000" border="1" style="FONT-SIZE: x-small;border-bottom-style:none;border-left-style:none" bordercolorlight="#000000" class="priceclass"><tbody><tr style="HEIGHT: 25px"><td style="BORDER-LEFT: solid thin">Item</td><td>Term</td><td>Monthly fee</td><td>Total for term $CAD</td></tr><tr style="HEIGHT:25px"><td style="BORDER-LEFT: solid thin">Basic</td><td>1 Month Term</td><td>39.95</td><td>39.95</td></tr>
...
if($shipable=="Y"){
//echo '		active[20]=false;';
}
echo '		CheckAll[21]= "USERNAME";
...
if($shipable!="Y"){
echo	'	if (myform.SHIP_DSL_M.value==\'---selec-one---\')
		{
...
/**********************************************Chris 2014-05-30 Adapters(not done)****************************/
                ...
                else
                {
                        ...
                        if(base>0)
                        {
                                ...
                                {
                                        tc[8][3]=roundit(tc[8][3]*myform.ADAPTERCOUNT.value,2);
                                }
                                tc[8][1]=\'One time\';
                                tc[8][2]=\'One time\';
                                if(tc[5][3]>0){
...
echo '
		...
		if (s1=="")
		{
			...
			if (taxpercent!=-1)
			{
				tc[3][3]=roundit((1.0*tc[1][3]+1.0*tc[2][3]+1.0*tc[8][3]+1.0*tc[5][3]+1.0*tc[6][3]+1.0*tc[7][3])*taxpercent/100.0,2);
				tc[4][3]=roundit(1.0*tc[1][3]+1.0*tc[2][3]+1.0*tc[8][3]+1.0*tc[5][3]+1.0*tc[3][3]+1.0*tc[6][3]+1.0*tc[7][3],2);
			}
		}
		else
		{
