...
WHERE   CONCAT( l.status,l.result) = lo.tstatus
        AND l.id <= lo.mid)
UNION DISTINCT
( SELECT ax.* from (select eid from agent_cache order by updatetime desc limit 40) as ec, agentcx ax where ec.eid=ax.id )
ORDER BY case when status='failed' then 2 else 1 end, status, result,id
";
}else{
...
//	$whitespp = mysql_real_escape_string($whitespp); 
	...
	$dbsql2="SELECT * from agentcx where phone like '%$pattern%' OR phone2 like '%$pattern%' OR phone like '%$hyphenp%' OR phone2 like '%$hyphenp%' ORDER BY case when status='failed' then 2 else 1 end, status, result,id limit 100";
...
//print "<pre>$dbsql2</pre>";
	...
	$dbsql2="SELECT * from agentcx where name like '%$pattern%' OR phone like '%$pattern%' OR phone2 like '%$pattern%' ORDER BY case when status='failed' then 2 else 1 end, status, result,id limit 100";
...
if ($db2->query($dbsql2))
{
		...
		if($_POST["sel"]!="" && $_POST["sel"]!="go"){
//			addcache($eid,$agent_id,$pattern);
		}
...
//                      $btn .= '<button type="button" class="btn btn-mini btn-success funcbtn" id="oncall'.$eid.'"></button>';
			...
			if($callagent_id == $agent_id){
				...
				if($utype=="main"){
					...
					if($resstr=='<span class="label label-success"> Submiting Order </span>'){
						$btn .= '&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-xs btn-success funcbtn fsubmt" id="fsubmt'.$eid.'"><span class="glyphicon glyphicon-check"></span>&nbsp;&nbsp;Force Submission </button>';
					}
				}else{
					$btn = $callingstr;
				}
