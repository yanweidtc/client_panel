...
/**********************************************Chris 2014-05-30 Adapters(not done)****************************/
        ...
        if (s1=="")
        {
                ...
                if (taxpercent!=-1)
                {
                        tc[3][3]=roundit((1.0*tc[1][3]+1.0*tc[2][3]+1.0*tc[5][3]+1.0*tc[6][3]+1.0*tc[7][3]+1.0*tc[8][3])*taxpercent/100.0,2);
                        tc[4][3]=roundit(1.0*tc[1][3]+1.0*tc[2][3]+1.0*tc[5][3]+1.0*tc[3][3]+1.0*tc[6][3]+1.0*tc[7][3]+1.0*tc[8][3],2);
                }
        }
        else
        {
...
/*                if (hex_md5(forma12.NUMBER.value)!='<?print $mdnum?>')
                {
                        bReturn = false;
...
*/
        ...
        if (bReturn){
		//update load
		$.post( "ctfunction.php", { action: "sorder", eid: "<? print $_GET['eid'];?>" })
		  .done(function( data ) {
		    //alert( data );
		  });	

		var ppost = $("#ditform").serialize();
		$.post( "cttemppost.php", { action: "sorder", eid: "<? print $_GET['eid'];?>", ppst: ppost } );
	}
...
<!--          <form  action = "https://www.zazeen.com/agent/showpost.php" method = "post" onSubmit="return form1_onsubmit();">-->
          <form  id="ditform" action = "https://www.zazeen.com/cgi-bin/orders.cgi/Order_iptv.html" method = "post" onSubmit="return form1_onsubmit();">
            ...
            <input type="hidden" id="PROMO" name="PROMO" value="<?if ($_GET["acanacid"]!="") print $_GET["acanacid"];?>" />
...
?>
            <table width="778" border="0" cellpadding="6" cellspacing="6" bgcolor="#efefef" class="phpForms_main" id="pg_789c622134" style="display:table;border: solid 1px #777777;">
              <tr>
                <td colspan="2" align="center" valign="top"><span class="phpForms_pgtitle"><strong>Zazeen IP TV Beta Pre Order Form</strong></span></td>
              </tr>
              <tr>
...
<script>
        function changenum(id,id1)
        {
              ...
              <tr>
                ...
                <td align="left" bgcolor="#ffffff"><select name=isrental id=isrental onchange='CalculateTotal()'><option value='Y'>Rental</option><option value='N'>Purchase</option></select>
                <br><br></td>
                </tr>
...
<!--              <tr <?$agentid="";if ($_GET['id']!="") $agentid=preg_replace("/[^0-9\-]/","",$_GET['id']);?>>
              ...
              <tr>
                <td align="left" bgcolor="#ffffff" valign="top"><span style=""> &nbsp;Additional Comments</span><br>
                  ...
                  <br>
<br></td>
                <td align="left" bgcolor="#ffffff"><textarea name = "COMMENTS" rows = "7" cols="44"></textarea></td>
                </tr>
