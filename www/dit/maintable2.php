...
//Chris, Number Pool
if($_POST["sel"]=="go"){
$dbsql2="select id, name, phone, phone2, email , status, callagent, agentid, result, callbefore, callafter, note, callnum, unlockflag,company from agentcx where !( status='oncall' and agentid='$agent_id' ) order by case when status='failed' then 2 else 1 end, status, result limit 300";
/*$dbsql2="SELECT  l.*
FROM    (
        SELECT  status,
                COALESCE(
...
WHERE   l.status >= lo.status
        AND l.status <= lo.status
        AND l.id <= lo.mid
