...
if(login_check($mysqli,false) == true) {
	...
	$new_ctr = 0;
		if ($qstmt = $mysqli->prepare("SELECT id,processed, valid, time, counter FROM members_login WHERE aid = ? AND type = 1 ORDER BY time desc limit 1")) {
		  ...
		  $qstmt->store_result();
		   ...
		   if($qstmt->num_rows > 0) { // If queue found
			...
			if($processed=="done"){
				if($valid=="Y"){
					 if($counter < 5){
					 	print '<span class="label label-success">Success</span>&nbsp;&nbsp;<a href="update_info.php" id="upd-btn" class="btn btn-mini btn-success">Update Info</a>';
						$new_ctr = $counter + 1;
					 }else{
						$new_ctr = 5;
					 	print $default;
					 ...
					 print '<span class="label label-important">Failed</span>&nbsp;&nbsp;<a href="update_info.php" id="upd-btn" class="btn btn-mini btn-success">Update Info</a>';
				}
				...
				if($counter < 5){
					print '<span class="label label-warning">Skipped due to too many requests</span>&nbsp;&nbsp;<a href="update_info.php" id="upd-btn" class="btn btn-mini btn-success">Update Info</a>';
					$new_ctr = $counter + 1;
	...
	log_bad("Tried direct access refreshcc/refresh",$mysqli);
}
